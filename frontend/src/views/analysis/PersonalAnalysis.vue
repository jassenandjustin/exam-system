<script setup>
/**
 * 个人学习分析（原 Analysis.vue 内容迁移）：
 *  - props.userId 指定分析对象（本人或教师/管理员钻取的学生）
 *  - 修复「各学科练习量与正确率」图表：图例置顶、containLabel、x 轴斜排
 */
...
async function loadAll() {
  ...
  try {
    const [s, t, sub, w, td, rep, rec] = await Promise.all([
      api.get(`/analysis/stats/${props.userId}`, { params: { days: periodDays.value } }),
      api.get(`/analysis/trend/${props.userId}`, { params: { days: periodDays.value } }),
      api.get(`/analysis/subject-analysis/${props.userId}`),
      api.get(`/analysis/weak-points/${props.userId}`, { params: { limit: 8 } }),
      api.get(`/analysis/type-distribution/${props.userId}`),
      api.get(`/analysis/report/${props.userId}`, { params: { type: reportType.value } }),
      api.get(`/analysis/recommend/${props.userId}`, { params: { count: 6 } })
    ])
    stats.value = s.data
...
async function refreshReport() {
  try {
    const { data } = await api.get(`/analysis/report/${props.userId}`,
...
const subjectBarOption = computed(() => {
  ...
  return {
    ...
    // 图例置顶，给学科名留出旋转空间（照抄 trendOption 的已验证模式）
...
<template>
  <div class="personal-analysis" v-loading="loading">
    <div class="page-head">
      <h3>个人学习情况</h3>
